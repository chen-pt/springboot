$(document).ready(function(){
    ...
    $(".theme_linker_btn").live("click",function(){
      productObj.source_btn = $(this).attr("data-desc");
      loadEvt.loadThemeDialog();
    });
    ...
    $(".add_one_goods_btn").live("click",function(){
        ...
        var baseUrl = "/single/product";

        if(productObj.source_btn == "goods_url"){
            $('[data-desc="'+productObj.source_btn+'"]').parents("form").find("[name='pic-text-url']").val(baseUrl+"?goodsId="+tmp_goods_id);
        }else if(productObj.source_btn == "goods_url_dialog"){
            $('[data-desc="'+productObj.source_btn+'"]').parents("form").find("[name='dialog_link_url']").val(baseUrl+"?goodsId="+tmp_goods_id);
        }
        $("#modal-goods-render").modal('hide');
    });

    $(".add_one_theme_btn").live("click",function(){
      var tmp_theme_id  = $(this).parent().find('[name="theme_id"]').val();
      var baseUrl = "/activity/activityDetail";

      if(productObj.source_btn == "theme_url"){
        $('[data-desc="'+productObj.source_btn+'"]').parents("form").find("[name='pic-text-url']").val(baseUrl+"?activeId="+tmp_theme_id);
      }else if(productObj.source_btn == "theme_url_dialog"){
        $('[data-desc="'+productObj.source_btn+'"]').parents("form").find("[name='dialog_link_url']").val(baseUrl+"?activeId="+tmp_theme_id);
      }

      $("#modal-theme-render").modal('hide');
    });
    ...
    $(".save-goods-btn").click(function(){
        ...
        if(productObj.source_btn == "goods_name"){
            ...
            $(".select_goods_list [name='goods_id']").each(function(){
                tmpData[num] = {};
                tmpData[num].goodsTitle = $(this).parent().find('[name="goods_title"]').val();
                tmpData[num].shopPrice = $(this).parent().find('[name="shop_price"]').val();
                tmpData[num].specifCation = $(this).parent().find('[name="specif_cation"]').val();
                tmpData[num++].goodsId = $(this).val();
            });
    ...
    $(".save-pic-btn").click(function(){
        ...
        var imgsrc = $(".tmp_pic_a img").attr("src").replace(/[0-9]+x[0-9]+/,'300x300');
...
var loadEvt = {
    ...
    loadThemeDialog:function(){
      $("#modal-theme-render").modal('show');
      productObj.parent_form = $("#modal-theme-render");
      $(".select_theme_table_one").removeClass("lee_hide");
      productObj.getThemeLst();
    },
...
var productObj = {
    ...
    getProductList:function(){
        ...
        if(goods_code)params.goodsCode = goods_code;
        ...
        params.startRow = (productObj.current_goods_page-1) * productObj.per_goods_page;
        ...
        $.post(urlStr+"?"+$.param(params),function(data){
            ...
            if(productObj.dialog_type == 1){
                ...
                var doTtmpl = doT.template(tmpl);
                $("#select_goods_one_list").html(doTtmpl(data.goodsPage));
                $("#modal-goods-render .modal-footer").addClass("lee_hide");
            }
            if(data.goodsPage.size>0){
                productObj.total_goods_page = data.goodsPage.prePage;
    ...
    getProductCategory:function(menu_num){
        if(menu_num == 1){
            ...
            var parent_id = $(productObj.parent_form).find('[name="cart_id_first"] [value="'+$(productObj.parent_form).find('[name="cart_id_first"]').val()+'"]').attr("data-id");
           ...
           $(productObj.parent_form).find('[name="cart_id_third"]').html('<option value="">全部分类</option>');
            ...
            var parent_id = $(productObj.parent_form).find('[name="cart_id_second"] [value="'+$(productObj.parent_form).find('[name="cart_id_second"]').val()+'"]').attr("data-id");
    ...
    cateHasGoods:function(cate_id){
      ...
      $.post(urlStr,params,function(data){
            if(data.total<1 || data.goodsPage.list.length<1){
                alert("该分类没有商品，无法保存。");
                return false;
            }

            var baseUrl = "/new/searchResult/";
            ...
            if(productObj.source_btn == "cate_name"){
                ...
                $('[data-desc="'+productObj.source_btn+'"]').parents("form").find("[name='pic-text-url']").val(baseUrl+"?userCateid="+productObj.cur_cate_id+"&oneC="+productObj.cur_cate_name);
            }else if(productObj.source_btn == "cate_url_dialog"){
                $('[data-desc="'+productObj.source_btn+'"]').parents("form").find("[name='dialog_link_url']").val(baseUrl+"?userCateid="+productObj.cur_cate_id+"&oneC="+productObj.cur_cate_name);
            }
    ...
    getThemeLst:function(){
      var params = {};

      var params = {};
      params.type = 1;
      $(".tile .selected").removeClass("selected");
      $.post("/merchant/theme/getDataLst",params,function(res){
        console.log(res);
        if(res.status == "OK"){
          var data = res.data;
          var tmpl = document.getElementById('product_list_select_one_theme').innerHTML;

          var doTtmpl = doT.template(tmpl);

          $("#select_theme_one_list").html(doTtmpl(data));
        }else{
          $("#select_theme_one_list").html("暂无数据");
        }
