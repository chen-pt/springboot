...
var getPandianList = function (pageNum, pageSize) {//获取各个参数
  ...
  console.log("搜索条件:盘点单号:[" + pandian_num + "],");
  $.ajax({
      async: false,
      type: 'post',
...
var getPanDianDetail = function (pandianOrderId, pandian_num, storeId, storeName, storeNumber, createTime, pandianType,
  ...
  $("#detailed [name=detail_pandian_num]").val(pandian_num);
  $("#detailed [name=detail_storeId]").val(storeId);
  ...
  getDetail_pandian(pandian_num, storeId, $("#detailed [name=profitOrLossStatu]").val());
}
//点击详情页面的搜索按钮
...
var getDetail_pandian = function (pandian_num, storeId, profitOrLossStatu) {
  ...
  console.info("门店后台盘点表盘点详情搜索详情:盘点单号:" + pandian_num + ",门店id:" + storeId + "," +
    "盘点状态:" + profitOrLossStatu + ",药品名称:" + drug_name + ",商品编码:" + goods_code);
  $.ajax({
    async: false,
    type: 'post',
    url: '/store/check/pandianDetail',
    dataType: 'JSON',
    data: {
      "pandian_num": pandian_num,
      "storeId": storeId,
    ...
    success: function (data) {
      ...
      if (data.status == 1) {
        ...
        detail_totalNum = data.data.total;
...
var detail_start2Repeat = function () {
  ...
  if (num) {//用户选中了对应的盘点单号
    $.ajax({
      ...
      type: "POST",
      dataType: "JSON",
      url: "/store/check/analyseTheTask",//复盘任务
      data: {
        "nums": num,
        "pandianType": "500"
      },
...
var detail_start2Confirm = function () {
  ...
  if (num) {//用户选中了对应的盘点单号
    $.ajax({
      ...
      type: "POST",
      dataType: "JSON",
      url: "/store/check/inspectDishTask",//监盘
      data: {
        "nums": num,
        "pandianType": "300"
      },
...
var detail_start2Audit = function () {
  ...
  if (num) {//用户选中了对应的盘点单号
    $.ajax({
      ...
      type: "POST",
      dataType: "JSON",
      url: "/store/check/reviewTheTask",//审核盘点任务
      data: {
        "nums": num,
        "pandianType": "400"
      },
...
var detail_export = function () {
  ...
  detailed_Data.storeId = $("#detailed [name=detail_storeId]").val();
  ...
  $.ajax({
    ...
    type: "POST",
    dataType: "JSON",
    url: "/store/check/reportBef",//导出盘点信息
    data: detailed_Data,
    success: function (data) {
...
var erpdata_get = function () {
  ...
  $.ajax({
    ...
    type: "POST",
    dataType: "JSON",
    url: "/store/check/erpData_get",//更新盘点数据
    data: {
...
function syncErpPandianData(orderId, panDianNum, storeId, unit_no) {
  ...
  $.ajax({
    ...
    type: "POST",
    dataType: "JSON",
    url: "/store/check/erp_initPanDianData",//更新盘点数据
    data: {
      "orderId": orderId,
      "panDianNum": panDianNum,
      "storeId": storeId,
