...
$(document).on("click",".downData",function(){
  $(".dataForm").hide();
  $('[name="type_id_all_trades"]').parent().removeClass("checked");
  $('[name="type_id_all_trades"]').prop("checked", false);
  $('[name="type_id"]').parent().removeClass("checked");
  $('[name="type_id"]').prop("checked", false);
  $('[name="goods_limit"]').parent().removeClass("checked");
  $('[name="goods_limit"]').prop("checked", false);
  $('#goods_select').hide();
  $('#goods_num').hide();
  console.log($('[name="goods_limit"]:checked').val());
});
...
var getTaskData = function () {
  ...
  //数据来源
  var task_source = $('[name="channel_id"]:checked').val();
...
var getRewardType = function(rewardType,targetId){
  ...
  //是人民币就 *100
  rewardDetail.limit = (rewardType == 10 && limit ? (limit * 10*10) : limit);
  rewardDetail.punish = punish;
  //当条件是关于金额的，要以金额计算，当奖励和封顶是人民币的话，以金额计算
  rewardDetail.punishLimit = ((punishLimit && rmbConditionTargetIds.indexOf(targetId)>-1)  ? (punishLimit * 10*10) : punishLimit);
  //仅仅与人民币还是绩效相关
  rewardDetail.penalize = (rewardType == 10 && penalize ? (penalize * 10*10) : penalize);
