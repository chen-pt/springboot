    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.code == 000){
              ...
              $("#infoId").append(doTtmpl(result));

              //本期应收
              var daishou=(result.total_pay==undefined?0:result.total_pay)-(result.refund_total==undefined?0:result.refund_total);
              var zhichu=(result.platform_total==undefined?0:result.platform_total)+(result.commission_total==undefined?0:result.commission_total)
                  +(result.post_total==undefined?0:result.post_total);
              // var yingshou=daishou-zhichu+(result.shangqijiezhuan==undefined?0:result.shangqijiezhuan);
              var yingshou=daishou-zhichu+(0);
              ...
              for(var i=0;i<result.list.length;i++){
                var temp=0;
                shouruA=shouruA+result.list[i].total_payA;
                tuikuanA=tuikuanA+result.list[i].refund_totalA;
